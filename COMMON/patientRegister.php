            ...
            <div class="col-md-12">
                ...
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb slider-breadcrumb">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">Patient SignUp
                </nav>
            </div>
        </div>
    </div>
                ...
                <div class="contact-form">
                    ...
                    <form class="form-details" method="post">
                        ...
                        <textarea placeholder="Address" class="form-control-textarea" name="address"
                            required></textarea>
                        <button type="submit" class="btn btn-Shop Message" name="patientReg">SignUp
...
if (isset($_REQUEST['patientReg'])) {
    ...
    if ($fetchData['cnt'] > 0) {
        echo "<script>alert('Already exists an Account with same Email/Phone');window.location='patientRegister.php';</script>";
    } else {
        $qryReg = "INSERT INTO `patient`(`name`,`email`,`phone`,`address`)VALUES('$Name','$Email','$Phone','$Address')";
        // echo $qryReg;
        $qryLog = "INSERT INTO `login` (`reg_id`,`email`,`password`,`usertype`,`status`) VALUES((SELECT MAX(`patient_id`) FROM `patient`),'$Email','$Password','Patient','Approved')";
        // echo $qryLog;
        if ($conn->query($qryReg) == TRUE && $conn->query($qryLog) == TRUE) {
            echo "<script>alert('Registration Success');window.location = 'login.php';</script>";
        } else {
            echo "<script>alert('Registration Failed');window.location = 'patientRegister.php';</script>";
        }
    }
}
