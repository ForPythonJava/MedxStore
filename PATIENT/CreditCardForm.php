...
if (isset($_REQUEST['payNow'])) {
  $uid = $_SESSION['uid']; // Assuming you're using session for user authentication
  $qry = "UPDATE `cart` SET `status`='Paid' WHERE `reg_id`='$uid' AND `status`='incart'";

  if ($conn->query($qry) === TRUE) {
    echo "<script>alert('Successfully Paid'); window.location = 'myOrders.php';</script>";
  } else {
