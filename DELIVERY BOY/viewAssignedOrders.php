...
<?php
$qry = "SELECT `cart`.*,`medicine`.*,`patient`.`name` AS `pat_name`,`patient`.`address` FROM `cart`,`medicine`,`patient` WHERE `cart`.`med_id`=`medicine`.`med_id` AND `patient`.`patient_id`=`cart`.`reg_id` AND `status`='Assigned' AND `cart`.`delivery_id`='$uid'";
