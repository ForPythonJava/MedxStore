...
<?php
$qry = "SELECT `cart`.*,`medicine`.*,`patient`.`name` AS `pat_name`,`patient`.`address` FROM `cart`,`medicine`,`patient` WHERE `cart`.`med_id`=`medicine`.`med_id` AND `patient`.`patient_id`=`cart`.`reg_id` AND `status`='Delivered' AND `cart`.`delivery_id`='$uid'";
$result = mysqli_query($conn, $qry);
if (mysqli_num_rows($result) < 1) {
    ?>
    <center>
        <h1 id="nodata" class="m-3">No Orders Assigned</h1>
    </center>
    <?php
                    ...
                    <tr id="row{{ forloop.counter }}" style="text-align: center;">
                        ...
                        <td id="statusPay">
                            <?php if ($row['status'] == "Assigned") { ?>
                                <a href="./markDelivery.php?id=<?php echo $row['cartid'] ?>" class="btn btn-primary">Mark as
                                    Delivered</a>
